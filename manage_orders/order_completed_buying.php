...
<div class="table-responsive box-table mt-3">
	...
	<nav id="pagination-order-completed" aria-label="Completed order navigation">
	</nav>
</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		activeOrder(<?= $login_seller_id ?>, 'completed', <?= isset($homePerPage) ? $homePerPage : 10 ?>);

		//executes code below when user click on pagination links
		$("body #pagination-order-completed").on("click", ".pagination a", function(e) {
			e.preventDefault();
			var page = $(this).attr("data-page"); //get page number from link
			$('body #wait').addClass("loader");
			activeOrder(<?= $login_seller_id ?>, 'completed', <?= isset($homePerPage) ? $homePerPage : 10 ?>, page);
		})
	});
</script>