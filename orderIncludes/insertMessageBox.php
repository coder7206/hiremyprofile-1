<?php if ($order_status == "pending" or $order_status == "progress" or $order_status == "delivered" or $order_status == "revision requested") { ?>
  <div class="insert-message-box">
    ...
    <div class="float-right">
      ...
      $isOnline = isset($buyer_status) ? $buyer_status : (isset($seller_status) ? $seller_status : "Offline");
      ?>
      <p class="text-muted mt-1">
        <!-- <?php if ($seller_id == $login_seller_id) { ?>
          <?= ($buyer_user_name); ?>
          <span <?php if (check_status($buyer_id) == "Online") { ?> class="text-success font-weight-bold" <?php } else { ?> style="color:#868e96; font-weight:bold;" <?php } ?>>
            is <?= check_status($buyer_id); ?>
          </span> | Local Time
        <?php } elseif ($buyer_id == $login_seller_id) { ?>
          <?= ($seller_user_name); ?>
          <span <?php if (check_status($seller_id) == "Online") { ?> class="text-success font-weight-bold" <?php } else { ?> style="color:#868e96; font-weight:bold;" <?php } ?>>
            is <?= check_status($seller_id); ?>
          </span> | Local Time
        <?php } ?> -->
        <?= ($seller_user_name); ?>
        <span <?php if ($isOnline == "online") { ?> class="text-success font-weight-bold" <?php } else { ?> style="color:#868e96; font-weight:bold;" <?php } ?>>
          is <?= ucfirst($isOnline); ?>
        </span> | Local Time
