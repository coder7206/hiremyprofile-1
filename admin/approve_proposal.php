...
if (!isset($_SESSION['admin_email'])) {
  ...
  if (isset($_GET['approve_proposal'])) {
    ...
    $membershipData = get_memebership_data($proposal_seller_id);
    $pendingProposal = $membershipData['pending_gig'];
    if ($pendingProposal == 0) {
      echo "<script>alert('Membership gigs exceeds for this user!');</script>";
      echo "<script> window.open('index?view_proposals_pending','_self');</script>";
      exit;
    }
