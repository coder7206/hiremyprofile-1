...
if (!isset($_SESSION['admin_email'])) {
    ...
    if ($enquiry_id == 1) {
        ...
        $defendedId = $sender_id == $orderSellerId ? $orderBuyerId : $orderSellerId;
    ...
    if (isset($_POST['new_message']) && !empty($_POST['new_message'])) {
        ...
        if (!in_array($file_extension, $allowed) & !empty($file)) {
            ...
            if ($insert_support_ticket) {
                ...
                // ORDER DISPUTE SUPPORT
                if ($enquiry_id == 1) {
                    $data['to'] = $defende_email;
                    $data['user_name'] = $defended_user_name;
                    send_mail($data);
                }
                ...
                if ($notifierPlugin == 1) {
                    ...
                    // ORDER DISPUTE SUPPORT
                    if ($enquiry_id == 1) {
                        sendSmsTwilio("", $smsText, $defended_phone);
                    }
                }
                ...
                // ORDER DISPUTE SUPPORT
                if ($enquiry_id == 1) {
                    $insert_notification = $db->insert("notifications", array("receiver_id" => $defendedId, "sender_id" => "admin_$admin_id", "order_id" => $ticket_id, "reason" => "ticket_reply", "date" => $date, "status" => "unread"));
                }
