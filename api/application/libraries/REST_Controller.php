...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->_enable_xss = ($this
            ->config
            ->item('global_xss_filtering') === true);
        ...
        $supported_formats = $this
            ->config
            ->item('rest_supported_formats');
        ...
        $this
            ->lang
            ->load('rest_controller', $language, false, true, __DIR__ . '/../');
        ...
        if ($this
            ->config
            ->item('rest_ip_blacklist_enabled') === true
        ) {
        ...
        if (isset($this->{'_' . $this
            ->request->method . '_args'}) === false) {
            $this->{'_' . $this
                ->request->method . '_args'} = [];
        }
        ...
        $this->_get_args = array_merge($this->_get_args, $this
            ->uri
            ->ruri_to_assoc());

        // Try to find a format for the request (means we have a request body)
        $this
            ->request->format = $this->_detect_input_format();
        ...
        $this
            ->response->format = $this->_detect_output_format();

        // Which language should the data be returned in?
        $this
            ->response->lang = $this->_detect_lang();
        ...
        if ($this
            ...
            $this
                ->request->body = Format::factory($this
                    ->request->body, $this
                    ->request
                    ->format)
                ->to_array();
            ...
            $this->{'_' . $this
                ->request->method . '_args'} = $this
                ->request->body;
        }

        //get header vars
        $this->_head_args = $this
            ->input
            ->request_headers();

        // Merge both for one mega-args variable
        $this->_args = array_merge($this->_get_args, $this->_options_args, $this->_patch_args, $this->_head_args, $this->_put_args, $this->_post_args, $this->_delete_args, $this->{'_' . $this
            ->request->method . '_args'});
        ...
        if ($this
            ->config
            ->item('rest_database_group') && ($this
                ->config
                ->item('rest_enable_keys') || $this
                ->config
                ->item('rest_enable_logging'))
        ) {
            $this
                ->rest->db = $this
                ->load
        ...
        if ($this
            ->config
            ->item('rest_enable_keys') && $this->auth_override !== true
        ) {
        ...
        if ($this
            ...
            $this->response([$this
                ->config
                ->item('rest_status_field_name') => false, $this
                ->config
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_ajax_only')], self::HTTP_NOT_ACCEPTABLE);
        }

        // When there is no specific override for the current class/method, use the default auth value set in the config
        if ($this->auth_override === false && (!($this
            ->config
            ->item('rest_enable_keys') && $this->_allow === true) || ($this
            ->config
            ->item('allow_auth_and_keys') === true && $this->_allow === true))) {
            $rest_auth = strtolower($this
                ->config
                ->item('rest_auth'));
            switch ($rest_auth) {
                case 'basic':
    ...
    protected function preflight_checks()
    {
        ...
        if ($this->session->has_userdata('app-key')) {
            $mobApiKey = $this->session->userdata('app-key');
        } else {
            $mobApiKey = $this->db->get("general_settings")->row()->mobileApp_apiKey;
            $this->session->set_userdata('app-key', $mobApiKey);
        }
        $appKey = $this->input->get_request_header('x-api-key', TRUE);

        if ($appKey == "" || ($appKey != $mobApiKey)) {
            // return $this->response("Mobile APP Key Required.", 403);
            header("Content-Type: application/json");
            http_response_code(403);
            echo json_encode(['Mobile APP Key Required']);
            exit();
        }
    }
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this
            ...
            $this->response([$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_unsupported')], self::HTTP_FORBIDDEN);
        }

        // Remove the supported format from the function name e.g. index.json => index
        $object_called = preg_replace('/^(.*)\.(?:' . implode('|', array_keys($this->_supported_formats)) . ')$/', '$1', $object_called);

        $controller_method = $object_called . '_' . $this
            ->request->method;
        // Does this method exist? If not, try executing an index method
        if (!method_exists($this, $controller_method)) {
            $controller_method = "index_" . $this
                ->request->method;
        ...
        if ($this
            ->config
            ->item('rest_enable_keys') && $use_key && $this->_allow === false
        ) {
            if ($this
                ->config
                ->item('rest_enable_logging') && $log_method
            ) {
                $this->_log_request();
            }
            ...
            $this->response([$this
                ...
                ->item('rest_status_field_name') => false, $this
                ->config
                ->item('rest_message_field_name') => sprintf($this
                ->lang
                ->line('text_rest_invalid_api_key'), $this
                ->rest
                ->key)], self::HTTP_FORBIDDEN);
        }

        // Check to see if this key has access to the requested controller
        if ($this
            ->config
            ->item('rest_enable_keys') && $use_key && empty($this
                ->rest
                ->key) === false && $this->_check_access() === false
        ) {
            if ($this
                ->config
                ->item('rest_enable_logging') && $log_method
            ...
            $this->response([$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_api_key_unauthorized')], self::HTTP_UNAUTHORIZED);
        }
        ...
        if (!method_exists($this, $controller_method)) {
            $this->response([$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_unknown_method')], self::HTTP_METHOD_NOT_ALLOWED);
        }

        // Doing key related stuff? Can only do it if they have a key right?
        if ($this
            ->config
            ->item('rest_enable_keys') && empty($this
                ->rest
                ->key) === false
        ) {
            // Check the limit
            if ($this
                ->config
                ->item('rest_enable_limits') && $this->_check_limit($controller_method) === false
            ) {
                $response = [$this
                    ->config
                    ->item('rest_status_field_name') => false, $this
                    ...
                    ->item('rest_message_field_name') => $this
                    ->lang
                    ->line('text_rest_api_key_time_limit')];
            ...
            if ($authorized === false) {
                ...
                $response = [$this
                    ...
                    ->item('rest_message_field_name') => $this
                    ->lang
                    ->line('text_rest_api_key_permissions')];
                $this->response($response, self::HTTP_UNAUTHORIZED);
            }
        }
        ...
        elseif ($this
            ->config
            ->item('rest_limits_method') == "IP_ADDRESS" && $this
            ->config
            ->item('rest_enable_limits') && $this->_check_limit($controller_method) === false
        ) {
            $response = [$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_ip_address_time_limit')];
            $this->response($response, self::HTTP_UNAUTHORIZED);
        }
        ...
        try {
            ...
            if ($this
                ->config
                ->item('rest_handle_exceptions') === false
            ) {
                throw $ex;
            }
    ...
    public function response($data = NULL, $http_code = NULL, $continue = false)
    {
        ...
        if (!$isProfilingEnabled) {
            ...
            elseif ($data !== NULL) {
                ...
                if (method_exists(Format::class, 'to_' . $this
                    ->response
                    ->format)) {
                    // Set the format header
                    $this
                        ->output
                        ->set_content_type($this->_supported_formats[$this
                            ->response->format], strtolower($this
                            ->config
                            ->item('charset')));
                    $output = Format::factory($data)->{'to_' . $this
                        ->response->format}();
    ...
    {
        $default_format = (string)$this
            ->config
            ->item('rest_default_format');
    ...
    protected function _detect_output_format()
    {
        ...
        if ($this
            ->config
            ->item('rest_ignore_http_accept') === false && $http_accept !== NULL
        ) {
    ...
    protected function _detect_method()
    {
        ...
        if ($this
            ->config
            ->item('enable_emulate_request') === true
        ) {
            $method = $this
                ->input
                ->post('_method');
    ...
    protected function _detect_api_key()
    {
        ...
        $this
            ->rest->user_id = NULL;
        $this
            ->rest->ignore_limits = false;

        // Find the key from server or arguments
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this
            ->input
            ->server($key_name))) {
            if (!($row = $this
                ->rest
                ...
                ->where($this
                    ->config
                    ->item('rest_key_column'), $key)->get($this
                    ->config
                    ->item('rest_keys_table'))
                ->row())) {
                return false;
            }
            ...
            $this
                ->rest->key = $row->{$this
                    ->config
                    ->item('rest_key_column')};

            isset($row->user_id) && $this
                ->rest->user_id = $row->user_id;
            isset($row->level) && $this
                ->rest->level = $row->level;
            isset($row->ignore_limits) && $this
                ->rest->ignore_limits = $row->ignore_limits;
    ...
    protected function _log_request($authorized = false)
    {
        ...
        $is_inserted = $this
            ...
            ->insert($this
                ->config
                ->item('rest_logs_table'), ['uri' => $this
                ->uri
                ->uri_string(), 'method' => $this
                ->request->method, 'params' => $this->_args ? ($this
                ->config
                ->item('rest_logs_json_params') === true ? json_encode($this->_args) : serialize($this->_args)) : NULL, 'api_key' => isset($this
                ->rest
                ->key) ? $this
                ->rest->key : '', 'ip_address' => $this
                ->input
                ->ip_address(), 'time' => time(), 'authorized' => $authorized]);

        // Get the last insert id to update at a later stage of the request
        $this->_insert_id = $this
            ->rest
            ->db
            ->insert_id();
    ...
    protected function _check_limit($controller_method)
    {
        ...
        switch ($this
            ...
            default:
                $limited_uri = $this
                    ->uri
                    ->ruri_string();
                if (strpos(strrev($limited_uri), strrev($this
                    ->response
                    ->format)) === 0) {
                    $limited_uri = substr($limited_uri, 0, -strlen($this
                        ->response
                        ->format) - 1);
                }
                $limited_uri = 'uri:' . $limited_uri . ':' . $this
                    ->request->method; // It's good to differentiate GET from PUT
        ...
        $result = $this
            ->rest
            ->db
            ->where('uri', $limited_uri)->where('api_key', $api_key)->get($this
                ->config
                ->item('rest_limits_table'))
            ->row();
        ...
        if ($result === NULL) {
            ...
            $this
                ...
                ->insert($this
                    ->config
                    ->item('rest_limits_table'), ['uri' => $limited_uri, 'api_key' => $api_key, 'count' => 1, 'hour_started' => time()]);
        }
        ...
        elseif ($result->hour_started < (time() - $time_limit)) {
            ...
            $this
                ->rest
                ->db
                ->where('uri', $limited_uri)->where('api_key', $api_key)->set('hour_started', time())
                ->set('count', 1)
                ->update($this
                    ->config
                    ->item('rest_limits_table'));
        ...
        // They have called within the hour, so lets update
            ...
            $this
                ->rest
                ->db
                ->where('uri', $limited_uri)->where('api_key', $api_key)->set('count', 'count + 1', false)
                ->update($this
                    ->config
                    ->item('rest_limits_table'));
        }
    ...
    protected function _auth_override_check()
    {
        ...
        $auth_override_class_method_http = $this
            ->config
            ->item('auth_override_class_method_http');

        // Check to see if the override array is even populated
        if (!empty($auth_override_class_method_http)) {
            // check for wildcard flag for rules for classes
            if (!empty($auth_override_class_method_http[$this
                ->router
                ->class]['*'][$this
                ->request
                ->method])) {
                // None auth override found, prepare nothing but send back a TRUE override flag
                if ($auth_override_class_method_http[$this
                    ->router
                    ->class]['*'][$this
                    ->request->method] === 'none') {
                    return true;
    ...
    protected function _parse_put()
    {
        if ($this
            ...
            if ($this
                ...
                $this->_put_args = json_decode($this
                    ->input
                    ->raw_input_stream);
            }
        } else if ($this
    ...
    protected function _parse_head()
    {
        ...
        parse_str(parse_url($this
            ->input
            ->server('REQUEST_URI'), PHP_URL_QUERY), $head);
    ...
    protected function _parse_options()
    {
        ...
        parse_str(parse_url($this
            ->input
            ->server('REQUEST_URI'), PHP_URL_QUERY), $options);
    ...
    protected function _perform_ldap_auth($username = '', $password = NULL)
    {
        ...
        $ldap = ['timeout' => $this
            ->config
            ->item('timeout', 'ldap'), 'host' => $this
            ->config
            ->item('server', 'ldap'), 'port' => $this
            ->config
            ->item('port', 'ldap'), 'rdn' => $this
            ->config
            ->item('binduser', 'ldap'), 'pass' => $this
            ->config
            ->item('bindpw', 'ldap'), 'basedn' => $this
            ->config
            ->item('basedn', 'ldap'),];
    ...
    protected function _perform_library_auth($username = '', $password = NULL)
    {
        ...
        $auth_library_class = strtolower($this
            ->config
            ->item('auth_library_class'));
        $auth_library_function = strtolower($this
            ->config
            ->item('auth_library_function'));
    ...
    protected function _check_login($username = NULL, $password = false)
    {
        ...
        $valid_logins = $this
            ->config
            ->item('rest_valid_logins');

        if (!$this
            ->config
            ->item('auth_source') && $rest_auth === 'digest') {
            // For digest we do not have a password passed as argument
            return md5($username . ':' . $this
                ->config
                ->item('rest_realm') . ':' . (isset($valid_logins[$username]) ? $valid_logins[$username] : ''));
        }
    ...
    protected function _check_php_session()
    {
        ...
        if ($this
            ->config
            ->item('rest_ip_whitelist_enabled')
        ) {
            $this->_check_whitelist_auth();
        }
        ...
        if (!$this
            ...
            $this->response([$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_unauthorized')], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _prepare_basic_auth()
    {
        ...
        $http_auth = $this
            ->input
            ->server('HTTP_AUTHENTICATION') ?: $this
            ->input
        ...
        if ($username !== NULL) {
            ...
            if (strpos(strtolower($http_auth), 'basic') === 0) {
                ...
                list($username, $password) = explode(':', base64_decode(substr($this
                    ->input
                    ->server('HTTP_AUTHORIZATION'), 6)));
            }
        }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        if (strcasecmp($digest['response'], $valid_response) !== 0) {
            ...
            $this->response([$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_invalid_credentials')], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _check_blacklist_auth()
    {
        ...
        $pattern = sprintf('/(?:,\s*|^)\Q%s\E(?=,\s*|$)/m', $this
            ->input
            ->ip_address());
        ...
        if (preg_match($pattern, $this
            ...
            $this->response([$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_ip_denied')], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    {
        $whitelist = explode(',', $this
            ->config
            ->item('rest_ip_whitelist'));
        ...
        if (in_array($this
            ...
            $this->response([$this
                ...
                ->item('rest_message_field_name') => $this
                ->lang
                ->line('text_rest_ip_unauthorized')], self::HTTP_UNAUTHORIZED);
        }
    }
    ...
    protected function _log_access_time()
    {
        ...
        return $this
            ...
            ->update($this
                ->config
                ->item('rest_logs_table'), $payload, ['id' => $this->_insert_id]);
    }
    ...
    protected function _check_access()
    {
        ...
        if ($this
            ->config
            ->item('rest_enable_access') === false
        ) {
            return true;
        }

        // Fetch controller based on path and controller name
        $controller = implode('/', [$this
            ->router->directory, $this
            ->router->class]);
        ...
        $accessRow = $this
            ...
            ->where('key', $this
                ->rest
                ->key)
            ->where('controller', $controller)->get($this
                ->config
                ->item('rest_access_table'))
            ->row_array();
    ...
    protected function _check_cors()
    {
        ...
        $allowed_headers = implode(', ', $this
            ->config
            ->item('allowed_cors_headers'));
        $allowed_methods = implode(', ', $this
            ->config
            ->item('allowed_cors_methods'));

        // If we want to allow any domain to access the API
        if ($this
            ->config
            ->item('allow_any_cors_domain') === true
        ) {
            ...
            if (in_array($origin, $this
                ->config
                ->item('allowed_cors_origins'))) {
        ...
        if (is_array($this
            ->config
            ->item('forced_cors_headers'))) {
            foreach ($this
                ->config
                ->item('forced_cors_headers') as $header => $value) {
                header($header . ': ' . $value);
            }
        }
