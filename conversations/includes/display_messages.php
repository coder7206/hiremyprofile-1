					...
					<div class="card-body p-2">
						...
						<?php if ($offer_status == "active") { ?>
							...
							<?php } else { ?>
								...
								<button id="reject-offer" onclick="putID(<?= $message_offer_id; ?>, <?= $message_group_id; ?>)" data-toggle="modal" data-target="#exampleModal" class="btn btn-danger float-right mr-1">
									<?= $lang['button']['reject_offer']; ?>
								</button>
								<script>
						...
						<?php } elseif ($offer_status == "rejected") { ?>
							<button class="btn btn-danger rounded-0 mt-2 float-right" disabled>
								<?= $lang['button']['offer_rejected']; ?>
							</button>
						<?php } ?>
					</div>
					<!--- card-body Ends --->
