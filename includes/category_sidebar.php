...
<div class="card border-success mb-3">
  ...
  <div class="card-body box-shadow-cat-sb">
    ...
    <ul class="nav flex-column" id="proposal_category">
      ...
      ?>
        <li class="nav-item">
          <span class="nav-link <?php if ($cat_id == @$_SESSION['cat_id'] || $cat_id == @$child_parent_id) {
                                  echo "active";
                                } ?>" data-toggle="collapse" data-target="#cat_<?= $cat_id; ?>">
            <a class="text-success"> <?= $cat_title; ?> <a class="h5 text-success float-right">
                <i class="fa fa-arrow-circle-down"></i>
              </a></a>
          </span>
          <ul id="cat_<?= $cat_id; ?>" class="collapse pl-1 <?php if ($cat_id == @$_SESSION['cat_id'] || $cat_id == @$child_parent_id) {
                                                              echo "show";
                                                            } ?>">   
            <?php
...
<div class="card border-success mb-3">
  <div class="card-body pb-2 pt-2 box-shadow-cat-sb">
...
<div class="card border-success mb-3">
  <div class="card-body pb-2 pt-3 <?= ($lang_dir == "right" ? 'text-right' : '') ?> box-shadow-cat-sb">
    <ul class="nav flex-column">
      <li class="nav-item checkbox checkbox-success">
        <label>
          <input type="checkbox" value="1" class="get_instant_delivery" <?php if ($instant_delivery == 1) {
                                                                          echo "checked";
                                                                        } ?>>
  ...
  <div class="card-body box-shadow-cat-sb">
    <ul class="nav flex-column">
      ...
      if (isset($_SESSION['cat_id'])) {
        ...
        $get_proposals = $db->query("SELECT DISTINCT proposal_seller_id FROM proposals WHERE proposal_cat_id = :cat_id AND proposal_status = 'active'", ['cat_id' => $cat_id]);
     
      } elseif (isset($_SESSION['cat_child_id'])) {
        $child_id = $_SESSION['cat_child_id'];
        $get_proposals = $db->query("SELECT DISTINCT proposal_seller_id FROM proposals WHERE proposal_child_id = :child_id AND proposal_status = 'active'", ['child_id' => $child_id]);
    
      } else {
        // Default behavior if neither session variable is set
        // You may want to handle this case differently based on your application logic
        $get_proposals = $db->query("SELECT DISTINCT proposal_seller_id FROM proposals WHERE proposal_status = 'active'");
      ...
      while ($row_proposals = $get_proposals->fetch()) {
        ...
        ?>
          <li class="nav-item checkbox checkbox-success">
            <label>
              <input type="checkbox" value="<?= $seller_country; ?>" class="get_seller_country" <?php if (isset($sellerCountry["$seller_country"])) {
                                                                                                  echo "checked";
                                                                                                } ?>>
    ...
    <ul class="nav flex-column">
      ...
      ?>
          <li class="nav-item checkbox checkbox-success" data-country="<?= $seller_country; ?>">
            <label>
              <input type="checkbox" value="<?= $seller_city; ?>" class="get_seller_city" <?php if (isset($sellerCity["$seller_city"])) {
                                                                                            echo "checked";
                                                                                          } ?>>
  ...
  <div class="card-body box-shadow-cat-sb">
    <ul class="nav flex-column">
      ...
      ?>
          <li class="nav-item checkbox checkbox-success">
            <label>
              <input type="checkbox" value="<?= $delivery_id; ?>" class="get_delivery_time" <?php if (isset($delivery_time[$delivery_id])) {
                                                                                              echo "checked";
                                                                                            } ?>>
...
<div class="card border-success mb-3">
  ...
  <div class="card-body box-shadow-cat-sb">
    <ul class="nav flex-column">
      ...
      if (isset($_SESSION['cat_id'])) {
        $get_proposals = $db->query("SELECT DISTINCT level_id FROM proposals WHERE proposal_cat_id = :cat_id AND proposal_status = 'active'", array("cat_id" => $_SESSION['cat_id']));
      } elseif (isset($_SESSION['cat_child_id'])) {

        $get_proposals = $db->query("SELECT DISTINCT level_id FROM proposals WHERE proposal_child_id = :child_id AND proposal_status = 'active'", array("child_id" => $_SESSION['cat_child_id']));
      }
      ...
      ?>
          <li class="nav-item checkbox checkbox-primary">
            <label>
              <input type="checkbox" value="<?= $level_id; ?>" class="get_seller_level" <?php if (isset($seller_level[$level_id])) {
                                                                                          echo "checked";
                                                                                        } ?>>
  ...
  <div class="card-body box-shadow-cat-sb">
    <ul class="nav flex-column">
      ...
      ?>
          <li class="nav-item checkbox checkbox-primary">
            <label>
              <input type="checkbox" value="<?= $language_id; ?>" class="get_seller_language" <?php if (isset($seller_language[$language_id])) {
                                                                                                echo "checked";
                                                                                              } ?>>
