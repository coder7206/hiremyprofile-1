			...
			if ($seller_status == "block-ban") {
				...
				if ($select_seller) {
					...
					if (isset($_SESSION['sessionStart']) and $_SESSION['sessionStart'] === $row_seller->seller_user_name) {
						$update_seller_status = $db->update("sellers", array("seller_status" => 'online', "seller_ip" => $ip, "device_type" => $device_type), array("seller_user_name" => $row_seller->seller_user_name, "seller_pass" => $hashed_password));
