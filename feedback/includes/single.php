	...
	<div class="card-body <?= $textRight; ?>">
		...
		<ul class="list-unstyled mt-4 text-left">
			...
			while ($comment = $comments->fetch()) {
				if ($comment->seller_id ==0) {
					$admin = $db->select("admins", ["admin_id" => $comment->admin_id])->fetch();
					$commenterName = $admin->admin_name;
					if (!empty($admin->admin_image))
						$commenterImg = getImageUrl("admins", $admin_image);
					else
						$commenterImg = $site_url . "/user_images/empty-image.png";
				} else {
					$seller = $db->select("sellers", ["seller_id" => $comment->seller_id])->fetch();
					$commenterName = $seller->seller_user_name;
					$commenterImg = getImageUrl2("sellers", "seller_image", $seller->seller_image);
				}

			?>

				<li class="media mb-3">
					<img class="mr-3 img-thumbnail" src="<?=$commenterImg;?>" width="50">
