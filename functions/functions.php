...
function freelancersQueryWhere($type)
{
		...
		foreach ($_REQUEST['online_sellers'] as $value) {
			if ($value != 0) {
				foreach ($online_sellers as $seller_id) {
					$i++;
					$where_online[] = "s.seller_id=:seller_id_$i";
	...
	if (isset($_REQUEST['seller_country'])) {
		...
		foreach ($_REQUEST['seller_country'] as $value) {
			$i++;
			if ($value != "undefined") {
				$where_country[] = "s.seller_country=:seller_country_$i";
	...
	if (isset($_REQUEST['seller_level'])) {
		...
		foreach ($_REQUEST['seller_level'] as $value) {
			$i++;
			if ($value != 0) {
				$where_level[] = "s.seller_level=:seller_level_$i";
	...
	if (isset($_REQUEST['seller_language'])) {
		...
		foreach ($_REQUEST['seller_language'] as $value) {
			$i++;
			if ($value != 0) {
				$where_language[] = "s.seller_language=:seller_language_$i";
	...
	if (isset($_REQUEST['seller_skills'])) {
		// Use implode() function to join
		$aSkills = $_REQUEST['seller_skills'];
...
function get_freelancers()
{
	...
	$query_where_skills = isset($query_where) ? $query_where[1] : "";
	$query_where = isset($query_where) ? $query_where[0] : "";
	...
	if (!empty($where_path)) {
		if ($query_where_skills != "") {
			$query = "SELECT s.* FROM sellers s INNER JOIN proposals p on s.seller_id = p.proposal_seller_id and p.proposal_status='active' INNER JOIN skills_relation sr on s.seller_id = sr.seller_id and s.seller_id INNER join seller_skills ss on sr.skill_id = ss.skill_id and ss.skill_id IN (" . $query_where_skills . ") $query_where GROUP by s.seller_id $where_limit;";
		} else {
			$query = "select DISTINCT s.* from sellers s JOIN proposals ON s.seller_id=proposals.proposal_seller_id and proposals.proposal_status='active' $query_where$where_limit";
		}
...
function get_freelancer_pagination()
{
	...
	if (!empty($where_path)) {
		if ($query_where_skills != "") {
			...
			$query = "select DISTINCT s.* from sellers s JOIN proposals ON s.seller_id=proposals.proposal_seller_id and proposals.proposal_status='active' $query_where";
		}
		$sellers = $db->query($query, $values);
	} else {
